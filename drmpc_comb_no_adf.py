...
x_real = np.array([0.0, 0.0, 0.0, 0.0])  # starting 
x_targ = np.array([3.0, 2.0, 0.0, 0.0])  # target
...
#     for i in range(k):
#         Ak[:, nx + i*nu : nx + (i+1)*nu] = \
#             np.linalg.matrix_power(A, k-1-i) @ B
#         Dk[:, i*nx:(i+1)*nx] = np.linalg.matrix_power(A, k-1-i)
...
U_sym = ca.MX.sym('u', nu, K)
X = opti.variable(X_sym)
U = opti.variable(U_sym)
eta = opti.variable(K)
lam = opti.variable(K)
s   = opti.variable(K, L)
...
def DRCVaR(obs_center, E, e, lifted_set, Hh_joint_set):
    ...
    for k in range(1, K+1):
        ...
        # n_xi = nx*k + nw
        # H_d_batch = np.kron(np.eye(k), H_d)   # diag(H_d, H_d, .... H_d)
        # h_d_batch = np.kron(np.ones(k), h_d)
        
        # H_joint = ca.MX.zeros(H_d_batch.shape[0] + H_w.shape[0], n_xi)
        # H_joint[:H_d_batch.shape[0], :nx*k] = H_d_batch
        # H_joint[H_d_batch.shape[0]:, nx*k:] = H_w
        # h_joint = ca.vertcat(h_d_batch, h_w)
        ...
        # risk_term = eta[k-1] + (1/beta)*(lam[k-1]*epsilon + (1/L)*ca.sum1(s[k-1,:]))
        # opti.subject_to(risk_term <= 0)
        # opti.subject_to(lam[k-1] >= 0)

        U_seq = ca.vertcat(*[U[:,i] for i in range(k)])
        P1 = ca.vertcat(x0_param, U_seq) 
        Q1 = Ak_hat @ P1 - e


        M_xi = ca.horzcat( Dk_hat, -E )
        ...
        for l in range(L):
            ...
            # U_seq = ca.vertcat(*[U[:,i] for i in range(k)])
            # P1 = ca.vertcat(x0_param, U_seq)                    # [x0 U_k|t].T
...
for n in range(num_obs):
    ...
    DRCVaR(obs_center, E[n], e[n], lifted_set, Hh_joint_set)
...
for t in range(T_sim):
    ...
    # dist_to_obs = np.linalg.norm(x_real[:2] - obs_center) - radius
    # distances_to_obstacle.append(dist_to_obs)
    ...
    if last_X_sol is not None:
        opti.set_initial(X, ca.horzcat(last_X_sol[:, 1:], last_X_sol[:, -1]))  # [X1 X2 ... X_{K-1}, X_{K}, X_{K}]
        opti.set_initial(U, ca.horzcat(last_U_sol[:, 1:], last_U_sol[:, -1]))

        if last_duals is not None:
            lam_v, eta_v, s_v, nu_list, gam_list = last_duals
        ...
        opti.set_initial(U, np.zeros((nu, K)))
    ...
    try:
        ...
        failed_flag = True
        solve_duration = time.time() - start_time
        print(f"At time {t} : Exception occured, Distance : {dist}")
        u_apply = np.zeros(nu)
        last_X_sol = None
